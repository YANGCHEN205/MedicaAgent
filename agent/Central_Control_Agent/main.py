...
from agent.Central_Control_Agent.prompt import gen_prompt, user_prompt,tools_map, query_user_for_details
...
class Central_Control_Agent(object):
    def __init__(self):
    ...
    def agent_execute(self, query, max_request_time=5, debug=False):
        ...
        while cur_request_time < max_request_time:
            ...
            if debug:
                print(f'-------------控制中心Agent第{cur_request_time}次推断------------')
        ...
        if cur_request_time == max_request_time:
            final_answer = "本次任务执行失败！未能提供闲聊任务"
        print(f"final_answer: {final_answer}")
        return final_answer


if __name__ == "__main__":
    ica_agent = Central_Control_Agent()
    max_request_time = 6
    query = '你好，我想要问诊一下，我有点感冒发烧'
    final_answer = ica_agent.agent_execute(query, max_request_time=max_request_time,debug=True)