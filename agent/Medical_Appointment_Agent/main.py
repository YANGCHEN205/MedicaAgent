...
from agent.Medical_Appointment_Agent.prompt import gen_prompt, user_prompt,tools_map, query_user_for_details
...
class Medical_Appointment_Agent(object):
    def __init__(self):
    ...
    def agent_execute(self, query, max_request_time=5, debug=False):
        ...
        while cur_request_time < max_request_time:
            ...
            if debug:
                print(f'-------------智能预约Agent第{cur_request_time}次推断------------')
...
if __name__ == "__main__":
    mka_agent = Medical_Appointment_Agent()
    max_request_time = 6
    query = '你好，我叫张三，我想预约今天的内科的李思医生'
