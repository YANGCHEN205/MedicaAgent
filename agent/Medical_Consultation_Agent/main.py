...
from agent.Medical_Consultation_Agent.prompt import gen_prompt, user_prompt,tools_map, query_user_for_details
...
class Medical_Consultation_Agent(object):
    def __init__(self):
    ...
    def agent_execute(self, query, max_request_time=5, debug=False):
        ...
        while cur_request_time < max_request_time:
            ...
            if debug:
                print(f'-------------智能诊断Agent第{cur_request_time}次推断------------')
...
if __name__ == "__main__":
    ...
    final_answer = mca_agent.agent_execute(query, max_request_time=max_request_time,debug=True)
    print(f"final_answer: {final_answer}")